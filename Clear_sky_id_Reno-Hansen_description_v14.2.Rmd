...
```{r echo=F, include=T}
library(data.table)
MS <- readRDS("PARAMS/Clear_sky_id_Reno-Hansen_apply_v14_2.Rds")
window            <-     MS$nt
alpha             <-     MS$alpha
ref_rm_VIP_RelLow <- 1 - MS$CS_ref_rm_VIP_RelLow
ref_rm_VIP_LowOff <-     MS$CS_ref_rm_VIP_LowOff
ref_rm_VIP_RelUpp <- 1 + MS$CS_ref_rm_VIP_RelUpp
ref_rm_VIP_UppOff <-     MS$CS_ref_rm_VIP_UppOff
```
...
The charactirization is done to the center value of the window.
...
Days with less than $`r window * 3`$ data points (minutes) will be ignored.
...
Comparing value in a moving window of $`r window`$ minutes.
The mean of the measured value $\overline{G}_i$ inside an envelope based on the reference model $\text{GHI}_\text{Clear Sky}$.


\begin{equation}
\overline{\text{GHI}}_{i\text{Clear Sky}} \times `r ref_rm_VIP_RelLow` - `r ref_rm_VIP_LowOff`
< \overline{G}_i <
\overline{\text{GHI}}_{i\text{Clear Sky}} \times `r ref_rm_VIP_RelUpp` + `r ref_rm_VIP_UppOff`
(\#eq:MeanVIP)
\end{equation}
